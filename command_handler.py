...
def handle_chat_command(
    ...
    if cmd in ["!슬롯확률", "!도박확률", "!확률", "!배당표", "!카지노확률", "!배당율"]:
        reply = (
            "🎰 [국고 슬롯 공식 확률 & 배당표]\n"
            "• 총 당첨률 31.1% (폭탄💣 32% 상향으로 국고 재정 건전화 완료)\n"
            "• 👑777: 국고 10%(최대 10만P) MEGA JACKPOT! (최소 10배)\n"
            "• 🀄역만: 6배 | 💎: 4배 | 🔔: 2.5배 | 🍇: 1.8배 | 🍒: 1.5배\n"
            "• 🥈2개 일치: 일반(🍒🍇🔔) 1.2배 적중(26.8%) | 고급(💎🀄7️⃣) 1.6배 적중(1.7%)\n"
            "• 💣/불일치: 꽝 (68.9% 국고 적립 | 1회 당첨 상한 10만P)"
        )
        return reply, None
    ...
    if cmd in ["!슬롯", "!슬롯머신", "!slot", "!도박", "!룰렛"]:
        if len(tokens) < 2:
            return "🎰 [슬롯머신] 사용법: !슬롯 [금액/올인] (예: !슬롯 1000, !슬롯 올인) | 777 대박 시 국고 10%(최대 10만P) 즉시 독식!", None
        bet_str = tokens[1]
