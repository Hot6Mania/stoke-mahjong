...
def handle_chat_command(
    ...
    if cmd in ["!슬롯확률", "!도박확률", "!확률", "!배당표", "!카지노확률", "!배당율"]:
        reply = (
            "🎰 [국고 슬롯 공식 확률 & 배당표]\n"
            "• 총 당첨률 45.9% (환급률 99.5% 🔥 주사위급 고수익률!)\n"
            "• 👑777: 국고 30% 독식 MEGA JACKPOT! (최소 20배 보장)\n"
            "• 🀄역만: 10배 | 💎: 6배 | 🔔: 4배 | 🍇: 2.5배 | 🍒: 2배\n"
            "• 🥈2개 일치: 일반(🍒🍇🔔) 2배 적중(38.5%) | 고급(💎🀄7️⃣) 2.5배 적중(2.8%)\n"
            "• 💣/불일치: 꽝 (54.1% 국고 적립)"
        )
        return reply, None
    ...
    if cmd in ["!슬롯", "!슬롯머신", "!slot", "!도박", "!룰렛"]:
        if len(tokens) < 2:
            return "🎰 [슬롯머신] 사용법: !슬롯 [금액/올인] (예: !슬롯 1000, !슬롯 올인) | 777 대박 시 국고 30% 즉시 독식!", None
        bet_str = tokens[1]
    ...
    if cmd in ["!주사위", "!다이스", "!dice"]:
        if len(tokens) < 3:
            return "🎲 [주사위 배틀] 사용법: !주사위 [홀/짝/대/소] [금액/올인] (예: !주사위 홀 2000, !주사위 대 올인) | 더블(1-1/6-6) 시 5배 대박!", None
